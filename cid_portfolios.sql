SELECT
  ...
  CASE
    WHEN PROJECTCLASS IN ('FIXED','BASELINE') THEN YTD_BUDGET_LABOR_COST
    ELSE ACTUAL_LABOR_COST
  END - YTD_BUDGET_LABOR_COST YTD_VARIANCE
...
SELECT 
  ...
  EXECUTIVE_MEMBERS.FULL_NAME EXECUTIVE_NAME,
  OBS_DEPT.NAME DEPARTMENT_NAME,
...
FROM
  ...
  WARM01.SRM_RESOURCES EXECUTIVE_MEMBERS,
  ...
  (SELECT O.UNIT_ID, O.NAME FROM OBS_UNITS_V O, PRJ_OBS_TYPES T WHERE O.TYPE_ID = T.ID AND O.DEPTH = 3 AND O.UNIT_MODE = 'OBS_UNIT_AND_ANCESTORS' AND T.ID = 5000001 GROUP BY O.UNIT_ID, O.NAME) OBS_DEPT,
...
WHERE 
  ...
  AND P_CONTENTS.PORTFOLIO_ID = PORTFOLIOS.ID
  ...
  AND DEPARTMENTS.OBS_UNIT_ID = OBS_DEPT.UNIT_ID
  AND DEPARTMENTS.BRM_ID = EXECUTIVE_MEMBERS.USER_ID(+)
